...
md"""
We will be using the `battery_w1` data from `pyextremes`, which measures hourly water elevation from 1 June 1920 to 31 March 2020.
"""
		...
		return combine(
			groupby(
				DataFrames.transform(
					X, 
					:date => ByRow(period) => :period
				),
				:period
			), 
			:water_elevation => maximum;
			renamecols = false
		)
	else
		return combine(
